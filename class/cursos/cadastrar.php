<?php
	...
	if($library->acessoRecusado('cursos.php', $_SESSION['idNivelServidor']) == FALSE){/* Excessão no caso do servidor não ter acesso a esta área*/
...
<div class='alert alert-success alert-dismissable' id="status" style="display:none">
		...
		<div class="col-sm-3">
			<select class="form-control" name="idNvlCurso" id="idNvlCurso">
				<?php
					...
					$sql = mysqli_query($library->databaseAcess->database->connection,$query);
					if(!$sql){
						die('erro: '.mysqli_error($library->databaseAcess->database->connection));
					}
